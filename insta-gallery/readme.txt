...
= 5.0.7 =
* Fix: OAuth reconnection now resets the renew-attempts counter and recalculates the token expiration date, recovering accounts stuck in a permanent renewal-blocked state
* Fix: Cron token renewal now sanitizes obfuscated tokens before persisting them
* Fix: Token expiration timestamps are computed in UTC to avoid timezone drift
* Fix: Account lookups no longer trigger renewal side effects that could push the attempts counter past the threshold
